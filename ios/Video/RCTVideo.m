...
- (void)adsLoader:(IMAAdsLoader *)loader failedWithErrorData:(IMAAdLoadingErrorData *)adErrorData {
    NSLog(@"OWAIS: ERROR in adsLoader, fallback to normal sream");
...
- (void)streamManager:(IMAStreamManager *)streamManager didReceiveAdEvent:(IMAAdEvent *)event {
    ...
    switch (event.type) {
        ...
        case kIMAAdEvent_CUEPOINTS_CHANGED: {
            NSLog(@"OWAIS: GOT EVENT: kIMAAdEvent_CUEPOINTS_CHANGED");
            if (_cuepoints == NULL) {
                _cuepoints = [event.adData objectForKey:@"cuepoints"];
            }
        }
        case kIMAAdEvent_AD_BREAK_STARTED: {
            NSLog(@"OWAIS: GOT EVENT: kIMAAdEvent_AD_BREAK_STARTED");
        ...
        case kIMAAdEvent_AD_BREAK_ENDED: {
            NSLog(@"OWAIS: GOT EVENT: kIMAAdEvent_AD_BREAK_ENDED");
    ...
    if (object == _playerItem) {
        ...
        if ([keyPath isEqualToString:statusKeyPath]) {
            ...
            if (_playerItem.status == AVPlayerItemStatusReadyToPlay) {
                ...
                if (self.onVideoLoad){
                    NSLog(@"OWAIS: onVideoLoad FOUND");
                }else{
                    NSLog(@"OWAIS: onVideoLoad NOT FOUND");
                }
                
                if (_videoLoadStarted){
                    NSLog(@"OWAIS: _videoLoadStarted YES");
                }else{
                    NSLog(@"OWAIS: _videoLoadStarted NO");
                }
                
                if (self.onVideoLoad && _videoLoadStarted) {
                    NSLog(@"OWAIS: onVideoLoad");
                    ...
                    if (self.onAdEvent && self->_cuepoints) {
                        ...
                        NSLog(@"OWAIS: CuePointsChange FIRED");
                    }
                }
